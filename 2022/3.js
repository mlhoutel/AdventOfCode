...
const rucksacs = input.split("\n");
const duplicates = rucksacs
  .map((r) => {
    const index = Math.floor(r.length / 2);
    return [r.slice(0, index), r.slice(index)];
  })
  .map(([a, b]) => {
    const first = new Set(a);
    return [...b].find((e) => first.has(e));
  });

const prios = duplicates.map((c) => {
  const indexes = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
  return indexes.indexOf(c) + 1;
});

console.log(prios.reduce((acc, val) => acc + val, 0));

const groups = [...new Array(rucksacs.length / 3)].map((_, i) =>
  rucksacs.splice(0, 3)
);
...
const badges = groups.map(
  (g) =>
    g
      .reduce(
        (acc, cur) => new Set([...cur].filter((v) => acc.has(v))),
        new Set(g[0])
      )
      .values()
      .next().value
...
const badges_prios = badges.map((c) => {
  const indexes = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
  return indexes.indexOf(c) + 1;
});

console.log(badges_prios.reduce((acc, val) => acc + val, 0));
