...
const values = lines.map((s) => {
  const nums = s.split("").filter((c) => "0123456789".includes(c));
  return `${nums[0]}${nums[nums.length - 1]}`;
});

console.log(values.reduce((acc, cur) => Number(cur) + acc, 0));
...
const values_replaced = lines.map((s) => {
  let nums = s;

  for (const [string, value] of Object.entries(digits)) {
    nums = nums.replace(new RegExp(string, "g"), value);
  }

  nums = nums.split("").filter((c) => "123456789".includes(c));

  return `${nums[0]}${nums[nums.length - 1]}`;
});

console.log(values_replaced.reduce((acc, cur) => Number(cur) + acc, 0));
