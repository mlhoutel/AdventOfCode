const input = `Game 1: 2 blue, 4 green; 7 blue, 1 red, 14 green; 5 blue, 13 green, 1 red; 1 red, 7 blue, 11 green
...
const total = (games.length * (games.length + 1)) / 2;

console.log(total - exclude.reduce((acc, val) => acc + val, 0));

const fewers = games.map((g) => {
  let mins = {
    red: 0,
    green: 0,
    blue: 0,
  ...
  for (const hand of g) {
    for (const c of hand) {
      ...
      if (mins[color] < Number(num)) mins[color] = Number(num);
    }
  }

  return Object.values(mins).reduce((acc, val) => acc * val, 1);
});

console.log(fewers.reduce((acc, val) => acc + val, 0));
